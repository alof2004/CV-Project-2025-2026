...
func _mat_summary(mat: Material) -> String:
	...
	if mat is StandardMaterial3D:
		...
		return "%s id=%s albedo_a=%.3f emission_on=%s emission_energy=%.3f blend=%s depth_draw=%s" % [
			t, id,
			sm.albedo_color.a,
...
func _apply_motion_safely(
	...
	for i in range(1, escape_steps + 1):
		var t := float(i) / float(escape_steps)
		if not _would_collide(grabbed_node, cs, motion * t, extra_offset):
			first_free_t = t
	...
	for _i in range(steps):
		var mid := (lo2 + hi2) * 0.5
		if _would_collide(grabbed_node, cs, motion * mid, extra_offset):
			lo2 = mid
		else:
			hi2 = mid
...
func _update_hover() -> void:
	...
	if pick_samples >= 5:
		var r := mouse_pick_radius_px
		offsets.append(Vector2( r, 0))
		offsets.append(Vector2(-r, 0))
		offsets.append(Vector2(0,  r))
		offsets.append(Vector2(0, -r))

	if pick_samples >= 9:
		var r2 := mouse_pick_radius_px * 0.7071
		offsets.append(Vector2( r2,  r2))
		offsets.append(Vector2(-r2,  r2))
		offsets.append(Vector2( r2, -r2))
		offsets.append(Vector2(-r2, -r2))
	...
	for off in offsets:
		var mp := mouse_pos + off

		var ray_origin := camera.project_ray_origin(mp)
		var ray_dir := camera.project_ray_normal(mp)
		var ray_to := ray_origin + ray_dir * pick_ray_length
		...
		var t := _get_target_from_collider(hit.get("collider"))
		if t == null:
			continue
	...
	if best_target != hovered:
		_dbg("HOVER %s -> %s" % [hovered.name if hovered else "NULL", best_target.name if best_target else "NULL"])
